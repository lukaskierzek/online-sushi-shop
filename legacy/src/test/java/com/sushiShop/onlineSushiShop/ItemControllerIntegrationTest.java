...
public class ItemControllerIntegrationTest {
    ...
    void getNonHiddenItems_shouldReturnListOfNonHiddenItemsGet() throws Exception {
        mockMvc.perform(get("http://localhost:8080/api/v1/onlinesushishop/raw/item/non-hidden"))
            ...
            .andExpect(jsonPath("$[0].itemActualPrice").value(10))
            .andExpect(jsonPath("$[1].itemActualPrice").value(10))
            .andExpect(jsonPath("$[0].itemOldPrice").value(20))
            .andExpect(jsonPath("$[1].itemOldPrice").value(100))
