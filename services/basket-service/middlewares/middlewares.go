...
var (
	errMessage = gin.H{"error": "An internal error occurred"}
)
...
func (m *CartMiddleware) CartHandlerFunc() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if cart == nil || cart.ID == "" || cart.CompleteDate != nil {
			...
			m.setCartIDCookie(host, c, newCart.ID)
		}
...
func (m *CartMiddleware) ensureCartIDCookie(host string, c *gin.Context) (string, error) {
	...
	if err != nil || cartID == "" {
		...
		cartID = newCart.ID
		m.setCartIDCookie(host, c, cartID)
	}
	return cartID, nil
}

func (m *CartMiddleware) setCartIDCookie(host string, c *gin.Context, cartID string) {
